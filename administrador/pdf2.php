<?php
  ...
  $pdf->SetX(40); // Mandamos una posicion en y
	...
	$pdf->Ln(10);
	$pdf->Cell(30,6,'Seccion',1,0,'C',1); // Imprimimos seccion
	$pdf->Cell(30,6,'Id de Pregunta',1,0,'C',1); // Imprimimos pregunta
	$pdf->Cell(65,6,'Pregunta',1,0,'C',1); // Imprimimos pregunta
	$pdf->Cell(65,6,'Respuesta correcta',1,1,'C',1); // Imprimimos Respuesta_correcta

	$pdf->SetFont('Arial','',7);

	while($row = $resultado->fetch_assoc()) // Arreglo asociativo
	{
		$pdf->Cell(30,6,utf8_decode($row['Materia']),1,0,'C'); // Imprimimos la materia
		$pdf->Cell(30,6,utf8_decode($row['Id_pregunta']),1,0,'C'); // Imprimimos la materia
		...
		if(strlen($row['Pregunta'])>60){
			...
			$resp .= "...";
			$pdf->Cell(65,6,utf8_decode($resp),1,0,'C');
		}else{
			$pdf->Cell(65,6,utf8_decode($row['Pregunta']),1,0,'C');
    }
    if(strlen($row['Respuesta_correcta'])>60){
			...
			$resp .= "...";
			$pdf->Cell(65,6,utf8_decode($resp),1,1,'C');
		}else{
			$pdf->Cell(65,6,utf8_decode($row['Respuesta_correcta']),1,1,'C');
		}
	}
